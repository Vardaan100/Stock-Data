...
def get_data_from_yahoo(reload_nifty50=False):
    ...
    if not os.path.exists('indianstock_dfs'):
        os.makedirs('indianstock_dfs')
    ...
    for ticker in tickers:
        ...
        if not os.path.exists('indianstock_dfs/{}.csv'.format(ticker)):
            try:
                df = web.DataReader(ticker,'yahoo', start, end)
                df.to_csv('indianstock_dfs/{}.csv'.format(ticker.replace(".NS","")))
...
def compile_data():
    ...
    for count,ticker in enumerate(tickers):
        df = pd.read_csv('indianstock_dfs/{}.csv'.format(ticker.replace(".NS","")))
